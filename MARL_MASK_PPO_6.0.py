...
def eval_action_mask(paziente, scenarios, tir_mean_dict, time_suffix, test_folder,
    ...
    if last_models:
        for m in os.listdir(os.path.join(main_train_folder, 'Training_'+p, f"Training_{p}_{soglia_ipo}_{soglia_iper}_{morty_cap}_{rick_cap}")):
            if m.startswith('T1DSimGymnasiumEnv_MARL_'+paziente):
                model = MaskablePPO.load(os.path.join(main_train_folder, 'Training_'+p, f"Training_{p}_{soglia_ipo}_{soglia_iper}_{morty_cap}_{rick_cap}", m.split('.')[0]))
    ...
    with pd.ExcelWriter(os.path.join(test_patient_folder, f'history_{paziente}_{soglia_ipo}_{soglia_iper}_{morty_cap}_{rick_cap}_{time_suffix_Min}.xlsx')) as patient_writer:
        ...
        for i, scen in zip(range(1,num_tests+1), scenarios.values()):
            ...
            placeholder_filename = os.path.join(test_patient_folder, f'results_{paziente}_{soglia_ipo}_{soglia_iper}_{morty_cap}_{rick_cap}_{time_suffix_Min}.xlsx')
        ...
        modified_filename = os.path.join(test_patient_folder, f'results_{paziente}_TIRmean_{round(time_in_range_mean, 2)}_{soglia_ipo}_{soglia_iper}_{morty_cap}_{rick_cap}_{time_suffix_Min}.xlsx')
        ...
        with pd.ExcelWriter(modified_filename) as final_writer:
            df_result.to_excel(final_writer, sheet_name='general_results', index=False)
    ...
    for train_timesteps, n_steps in zip(total_timesteps_list, n_timesteps_list):
        ...
        for p in (pazienti):
            ...
            for soglia_ipo, soglia_iper, morty_cap, rick_cap in itertools.product(soglia_ipo_list, soglia_iper_list, morty_cap_list, rick_cap_list):
                ...
                train_folder = os.path.join(main_train_folder, f"Training_{p}", f"Training_{p}_{soglia_ipo}_{soglia_iper}_{morty_cap}_{rick_cap}")
                
                test_patient_folder = os.path.join(test_folder, p)
                os.makedirs(test_patient_folder, exist_ok=True)
                
                start_time = datetime.strptime('3/4/2024 12:00 AM', '%m/%d/%Y %I:%M %p')
