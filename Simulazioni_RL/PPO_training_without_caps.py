...
# n_steps_list = [16, 64, 256, 1024] # 1 non si può
# tmstps_list = [16, 64, 256, 1024]

n_steps_list = [100000] # 1 non si può
tmstps_list = [100000]
...
for total_timesteps, n_steps in zip(tmstps_list, n_steps_list):
        ...
        for p, cap in list(opt_dict.items()):
            ...
            for c in cap:
                ...
                model.save(os.path.join(model_path, "ppo_nocaps_"+p+'_nsteps_'+str(n_steps)+'_total_tmstp_'+str(total_timesteps)+"_lr_"+str(learning_rate).replace('.','')+'_insmax'+str(c).replace('.',''))) # single train
