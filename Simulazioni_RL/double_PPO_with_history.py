...
# strategy_path = os.path.join(cwd, 'Risultati', 'Strategy')
...
with open(os.path.join(cwd, 'Strategy', 'scenarios_'+scenario_usato+'.json')) as json_file:
...
for training_n_steps in training_n_step_list:
    ...
    for k,v in opt_dict.items():
        ...
        with pd.ExcelWriter(os.path.join(cwd, 'Risultati', 'performance_double_ppo_with_risk_test_'+paziente+'_timesteps_'+str(test_timesteps)+'_training_nsteps_'+str(training_n_steps)+'_training_tmstp_'+str(training_total_timesteps)+'_ripetizioni_'+str(ripetizioni)+'.xlsx')) as writer:
