...
tir_dict = {'time in range mean':[],
            ...
            'severe hyper mean':[],
            'severe hypo mean':[],
            'cap severe iper mean':[],
            'cap iper mean':[],
            'cap ipo mean':[],
            'soglia severe iper mean':[],
            'soglia iper mean':[],
            'soglia ipo mean':[],
            'timesteps':[],
            'ripetizioni':[],
...
pazienti_list = ['adult#009']#, 'adult#010', 'adult#006']
# severe_iper_control_list = ['015', '016', '017', '018']
# iper_control_list = ['009','011','013']
# ipo_control_list = ['005']
# severe_iper_soglia_list = [230,250]
# iper_soglia_list = [165]
# ipo_soglia_list = [85]
# ripetizioni = 10

# per provare
severe_iper_control_list = ['013']
iper_control_list = ['009']
ipo_control_list = ['005']
severe_iper_soglia_list = [165]
iper_soglia_list = [125]
ipo_soglia_list = [85]
ripetizioni = 50
...
for paziente in pazienti_list:
    ...
    for severe_iper in severe_iper_control_list:
    
        for iper in iper_control_list:
            # print('iper cap: '+iper)
            for ipo in ipo_control_list:
                # print('ipo cap: '+ipo)
                for severe_iper_s in severe_iper_soglia_list:
                    for iper_s in iper_soglia_list:
                        # print('iper soglia: '+str(iper_s))
                        for ipo_s in ipo_soglia_list:

                    
                            print(paziente, severe_iper, iper, ipo, severe_iper_s, iper_s, ipo_s)
                            ...
                            for i in range(ripetizioni):
                                ...
                                model_ppo_severe_iper = PPO.load(os.path.join(model_path, 'ppo_sim_mod_food_hour_'+paziente+'_tmstp'+str(training)+'_lr00003_insmax'+severe_iper+'_customscen')) # iper
                                ...
                                for t in range(timesteps):
                                    ...
                                    if observation[0][0] < ipo_s:
                                        action = np.array([[0.0]]) # action = 0
                                    if observation[0][0] > severe_iper_s:
                                        action = model_ppo_severe_iper.predict(np.array(observation)) # severe iper control
                                ...
                                severe_iper_mod = insert_dot(severe_iper, 1)
                                tir_dict['cap severe iper'].append(severe_iper_mod)
                                iper_mod = insert_dot(iper, 1)
                                tir_dict['cap iper'].append(iper_mod)
                                ipo_mod = insert_dot(ipo, 1)
                                tir_dict['cap ipo'].append(ipo_mod)
                                tir_dict['soglia severe iper'].append(severe_iper_s)
                            ...
                            tir_mean_dict['cap severe iper mean'] = tir_dict['cap severe iper'][0]
                            tir_mean_dict['cap iper mean'] = tir_dict['cap iper'][0]
                            tir_mean_dict['cap ipo mean'] = tir_dict['cap ipo'][0]
                            tir_mean_dict['soglia severe iper mean'] = tir_dict['soglia severe iper'][0]
                            ...
                            df_final.to_excel(os.path.join(strategy_path,'triple_ppo_performance_'+paziente+'_'+str(timesteps)+'_allmeans_train1440(2048)_'+str(ripetizioni)+'ripetizioni.xlsx')
