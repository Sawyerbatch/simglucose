...
opt_dict = {
            # 'adult#001':('01','006',160,90, 'adult#008'),
            # 'adult#002':('01','006',160,90, 'adult#008'),
            # 'adult#003':('011','007',160,90, 'adult#007'),
            'adult#004':('014','007',160,90, 'adult#010'), # 'adult#004':('01','006',160,90, 'adult#008'),
            # 'adult#005':('009','006',160,90, 'adult#001'),
            # 'adult#006':('011','006',160,90, 'adult#003'),
            # 'adult#007':('011','006',160,90, 'adult#003'),
            # 'adult#008':('009','006',160,90, 'adult#001'),
            # 'adult#009':('011','006',160,90, 'adult#003'),
            # 'adult#010':('009','005',160,90, 'adult#004')
            }
...
for training_n_steps in training_n_step_list:
    ...
    for k,v in opt_dict.items():
        ...
        writer = pd.ExcelWriter(os.path.join(strategy_path,'performance_double_ppo_withcaps_safecontrol_real_patient_train_'+train+'_test_'+paziente+'_timesteps_'+str(test_timesteps)+'_training_nsteps_'+str(training_n_steps)+'_training_tmstp_'+str(training_total_timesteps)+'_ripetizioni_'+str(ripetizioni)+'.xlsx'))
        ...
        for i, scen in zip(range(ripetizioni), scenarios.values()):
            ...
            model_ppo_iper = PPO.load(os.path.join(model_path, 'ppo_withcaps_'+train+'_nsteps_'+str(training_n_steps)+'_total_tmstp_'+str(training_total_timesteps)+'_lr_00003_insmax'+iper)) # iper
            model_ppo_ipo = PPO.load(os.path.join(model_path, 'ppo_withcaps_'+train+'_nsteps_'+str(training_n_steps)+'_total_tmstp_'+str(training_total_timesteps)+'_lr_00003_insmax'+ipo))  # ipo 
        ...
        tir_mean_dict['paziente di test'].append(k)
        tir_mean_dict['paziente di train'].append(train)
