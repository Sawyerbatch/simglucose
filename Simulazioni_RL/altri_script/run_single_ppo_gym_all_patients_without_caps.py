...
training_n_step_list = [16, 64, 256, 1024]
training_total_timesteps = [16, 64, 256, 1024]

training_n_step_list = [100000]
...
for training_n_steps, training_total_timesteps in zip(training_n_step_list, training_total_timesteps) :
    ...
    writer = pd.ExcelWriter(os.path.join(strategy_path,'performance_single_ppo_nocaps_test_timesteps_'+str(test_timesteps)+'_training_nsteps_'+str(training_n_steps)+'_training_tmstp_'+str(training_total_timesteps)+'_ripetizioni_'+str(ripetizioni)+'.xlsx'))
    ...
    for k,v in opt_dict.items():
        ...
        for i, scen in zip(range(ripetizioni), scenarios.values()):
            ...
            model_ppo = PPO.load(os.path.join(model_path, 'ppo_nocaps_'+paziente+'_nsteps_'+str(training_n_steps)+'_total_tmstp_'+str(training_total_timesteps)+'_lr_00003_insmax'+cap))
