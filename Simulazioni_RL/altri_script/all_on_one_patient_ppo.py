...
df_final = pd.DataFrame(columns=list(tir_mean_dict.keys()))

for k,v in list(cap_ottimi.items()):
    model_ppo_iper = PPO.load(os.path.join(model_path, 'ppo_sim_mod_food_hour_'+k+'_tmstp'+str(training)+'_lr00003_insmax'+v[0]+'_customscen')) # iper
    model_ppo_ipo = PPO.load(os.path.join(model_path, 'ppo_sim_mod_food_hour_'+k+'_tmstp'+str(training)+'_lr00003_insmax'+v[1]+'_customscen')) # iper
                
      
    tir_dict = {'paziente usato':k,
                'time in range':[],
                'hyper':[],
                'hypo':[],
                'severe hyper':[],
    ...
    tir_mean_dict = dict()
    tir_mean_dict['paziente usato']= k
    ...
    df_final.to_excel(os.path.join(strategy_path,'performance_all_on_one_'+paziente+'_'+str(timesteps)+'_superppo_train1440(2048)_ripetizioni'+str(ripetizioni)+'.xlsx')
