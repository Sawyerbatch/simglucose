    ...
    for cap in cap_list:
        ...
        for i in range(ripetizioni):
            ...
            if patient_type == 'adult' and reward_type == 'new':
                model_ppo = PPO.load(os.path.join(model_path, "ppo_withcaps_"+paziente+'_nsteps_'+str(training)+'_total_tmstp_'+str(training)+'_lr_00003_insmax'+cap))  
            
            elif patient_type == 'adult' and reward_type == 'magni':
                model_ppo = PPO.load(os.path.join(model_path, "magni_ppo_withcaps_"+paziente+'_nsteps_'+str(training)+'_total_tmstp_'+str(training)+'_lr_00003_insmax'+cap)) 

            elif patient_type == 'adolescent': 
                model_ppo = PPO.load(os.path.join(model_path, "ppo_"+reward_type+"_reward_withcaps_"+paziente+'_nsteps_'+str(training)+'_total_tmstp_'+str(training)+'_lr_00003_insmax'+cap))  
            ...
            for t in range(timesteps):
                ...
                action = model_ppo.predict(np.array(observation)) # control
        ...
        tir_mean_dict['soglia mean'].append(tir_dict['soglia'][0])
        ...
        df_final.to_excel(os.path.join(results_path, 'single_ppo_'+reward_type+'_reward_'+paziente+'gridsearch_performance_'+str(timesteps)+'steps_'+str(training)+'('+str(training)+')training_'+str(ripetizioni)+'ripetizioni.xlsx')
