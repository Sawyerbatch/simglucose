...
with open(os.path.join('Strategy', 'scenarios_'+scenario_usato+'.json')) as json_file:
...
for training_n_steps, training_total_timesteps in zip(training_n_step_list, training_total_timesteps) :
    ...
    # writer = pd.ExcelWriter(os.path.join(cwd,'performance_single_ppo_withcaps_safecontrol_test_timesteps_'+str(test_timesteps)+'_training_nsteps_'+str(training_n_steps)+'_training_tmstp_'+str(training_total_timesteps)+'_ripetizioni_'+str(ripetizioni)+'.xlsx'))
    ...
    for k,v in opt_dict.items():
        ...
        with pd.ExcelWriter(os.path.join(cwd, 'Risultati', 'performance_single_ppo_with_risk_test_'+paziente+'_timesteps_'+str(test_timesteps)+'_training_nsteps_'+str(training_n_steps)+'_training_tmstp_'+str(training_total_timesteps)+'_ripetizioni_'+str(ripetizioni)+'.xlsx')) as writer:
            ...
            for i, scen in zip(range(ripetizioni), scenarios.values()):
                ...
                model_ppo = PPO.load(os.path.join(model_path, "ppo_withcaps_"+paziente+'_nsteps_'+str(training_n_steps)+'_total_tmstp_'+str(training_total_timesteps)+'_lr_00003_insmax'+cap))
