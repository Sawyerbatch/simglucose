...
if patient_type == 'adult' and reward_type == 'new':
    ...
    opt_dict = {
        ...
        'adolescent#004': ('006', 90),
        'adolescent#005': ('005', 90),
        'adolescent#006': ('005', 90),
        'adolescent#007': ('005', 90),
        'adolescent#008': ('007', 90),
        'adolescent#009': ('005', 90),
        'adolescent#010': ('006', 90),
    }
...
for training_n_steps, training_total_timesteps in zip(training_n_step_list, training_total_timesteps):
    ...
    for k, v in opt_dict.items():
        ...
        with pd.ExcelWriter(os.path.join(cwd, 'Risultati', 'performance_'+reward_type+'_reward_'+patient_type+'_random_policy_single_test_'+paziente+'_timesteps_'+str(test_timesteps)+'_training_nsteps_'+str(training_n_steps)+'_training_tmstp_'+str(training_total_timesteps)+'_ripetizioni_'+str(ripetizioni)+'.xlsx')) as writer:
            ...
            for i, scen in zip(range(ripetizioni), scenarios.values()):
                ...
                # elif patient_type == 'adult' and reward_type == 'magni':
                #     model_ppo = PPO.load(os.path.join(model_path, "magni_ppo_withcaps_"+paziente+'_nsteps_'+str(
                #         training_n_steps)+'_total_tmstp_'+str(training_total_timesteps)+'_lr_00003_insmax'+cap))

                # elif patient_type == 'adolescent':
                #     model_ppo = PPO.load(os.path.join(model_path, "ppo_"+reward_type+"_reward_withcaps_"+paziente+'_nsteps_'+str(
                #         training_n_steps)+'_total_tmstp_'+str(training_total_timesteps)+'_lr_00003_insmax'+cap))
                ...
                for t in range(test_timesteps):
                    ...
                    float_cap = int(cap) / 100

                    if observation[0][0] < ipo_s:
                        action = 0.0
                    else:
                        action = round(random.uniform(0, float_cap),4)
                    
                    ins_dict[paziente].append(action)
                ...
                cap_mod = insert_dot(cap, 1)
                tir_dict['cap'].append(cap_mod)
                ...
                df_res_mean = pd.DataFrame(tir_dict)

                df_res_mean.to_excel(writer, sheet_name=paziente, index=False)
            ...
            tir_mean_dict['cap mean'].append(tir_dict['cap'][0])
