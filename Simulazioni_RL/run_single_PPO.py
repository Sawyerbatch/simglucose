        ...
        with pd.ExcelWriter(os.path.join(cwd, 'Risultati', 'performance_'+reward_type+'_reward_'+patient_type+'_single_ppo_test_'+paziente+'_timesteps_'+str(test_timesteps)+'_training_nsteps_'+str(training_n_steps)+'_training_tmstp_'+str(training_total_timesteps)+'_ripetizioni_'+str(ripetizioni)+'.xlsx')) as writer:
            ...
            for i, scen in zip(range(ripetizioni), scenarios.values()):
                ...
                if patient_type == 'adult' and reward_type == 'new':
                    ...
                    model_ppo = PPO.load(os.path.join(model_path, "magni_ppo_withcaps_"+paziente+'_nsteps_'+str(training_n_steps)+'_total_tmstp_'+str(training_total_timesteps)+'_lr_00003_insmax'+cap))
                
                elif patient_type == 'adolescent': 
                    model_ppo = PPO.load(os.path.join(model_path, "ppo_"+reward_type+"_reward_withcaps_"+paziente+'_nsteps_'+str(training_n_steps)+'_total_tmstp_'+str(training_total_timesteps)+'_lr_00003_insmax'+cap))
