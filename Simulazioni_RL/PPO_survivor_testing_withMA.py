...
#   scenario = []
#   cho_sum = 0
#   mu_break, sigma_break = 8, 3 # 8,1 
#   mu_lunch, sigma_lunch = 13, 1 # 13,1
#   mu_snack, sigma_snack = 17, 2 # 17,1
#   mu_dinner, sigma_dinner = 21, 2 # 21, 1
#   mu_night, sigma_night = 24, 2 # 24,1

#   for i in range(n_days):

#     mu_cho_break, sigma_cho_break = cho_daily*0.15, 15 # 7
#     mu_cho_lunch, sigma_cho_lunch = cho_daily*0.45, 45 # 20
#     mu_cho_snack, sigma_cho_snack = cho_daily*0.05, 5 # 2
#     mu_cho_dinner, sigma_cho_dinner = cho_daily*0.35, 35 # 15
#     mu_cho_night, sigma_cho_night = cho_daily*0.05, 5 # 2
...
training_n_step_list = [32768]
# training_total_timesteps = [1024]
training_total_timesteps = [32768]

# test parameters
n_days = 1
n_hours = n_days*24
# test_timesteps = 2400 # 5 giorni
test_timesteps = 1440 # 1 giorno
...
for training_n_steps, training_total_timesteps in zip(training_n_step_list, training_total_timesteps) :
    ...
    for k,v in opt_dict.items():
        ...
        writer = pd.ExcelWriter(os.path.join(strategy_path,'performance_survivor_ppo_ripetizioni_'+k+'_'+str(ripetizioni)+'_ma_'+str(ma)+'_giorni_'+str(n_days)+'.xlsx'))
        ...
        for i, scen in zip(range(ripetizioni), scenarios.values()):
            ...
            for t in range(test_timesteps):
                ...
                action = moving_average(action_list, ma)    
